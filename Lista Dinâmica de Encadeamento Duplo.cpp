//Programa Lista Dinamica de Encadeamento Duplo
...
void inserir_inicio(int pvalor){
	...
	if (inicio == NULL){ //Insere para lista vazia
		...
		novo -> proximo = inicio;
		novo -> anterior = NULL;
		inicio -> anterior = novo; // Atualiza o no antigo
		inicio = novo;
	}
}
...
void inserir_meio(int pvalor){
	...
	//Verifica se o usuario quer inserir entre o inicio e o fim para lista com mais de 2 valores (Lida com repeticao)
	if ((inicio -> valor == a && fim -> valor == b || inicio -> valor == b && fim -> valor == a) && inicio -> proximo != fim){ 
		...
		if (aux == fim && aux -> anterior -> valor == b){ //Insere caso 'a' seja fim
		novo -> proximo = aux;
		novo -> anterior = aux -> anterior;
		aux -> anterior -> proximo = novo;
		aux -> anterior = novo;
		printf("\nValor inserido!\n");
			...
			printf("\nOs valores digitados ou nao existem na lista ou nao sao consecutivos\n");
		}
	}
}
...
void excluir(){
	...
	if(aux -> valor == i && aux -> proximo == NULL){ //Lista com 1 elemento
		free(aux);
		inicio = NULL;
		fim = NULL;
		printf("\nValor removido\n");
